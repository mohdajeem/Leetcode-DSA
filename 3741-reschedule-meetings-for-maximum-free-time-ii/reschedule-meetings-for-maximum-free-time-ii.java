class Solution {
    public int maxFreeTime(int eventTime, int[] startTime, int[] endTime) {
        ...
        for(int i = s-2;i>=0;i--){
            rightArr[i] = Math.max(rightArr[i+1], time.get(i+1));
        }
        for(int i = 1;i<s;i++){
            leftArr[i] = Math.max(leftArr[i-1], time.get(i-1));
        }
        int res = 0;
        for(int i = 1;i<s;i++){
            int diff = endTime[i-1]-startTime[i-1];
            if(diff <= Math.max(rightArr[i], leftArr[i-1])){
                res = Math.max(res, time.get(i-1)+diff+time.get(i));
            }
            res = Math.max(res, time.get(i-1)+time.get(i));
        }
